...
* <2017-07-29 Sat>
** Last error
   - Last error was line 249. The reason was simply that the when looking for a strong character I wasn't mapping the AN and EN to RTL as I  should.
   - Down to 0 errors! 
* <2017-07-31 Mon>
** Optimization
   - Created short-circuit pointers for searching for the next and previous isolate characters. This should speed up the algorithm quite a bit.
   - [X] TBD - The fall down to lower isolate levels in ~get_adjacent_run()~ is wrong! Need to return sentinel if this happens.
* <2017-11-07 Tue>
** Errors
   I don't understand what happened, but when retesting both the unicode-10 and my unicode-63 branch, I am back to 15 errors in the the character test! My plan is to solve this based on the dov-unicode63 branch as follows:
   - [X] Commit the deprecation fixes in the unicode-10 branch.
   - [X] Create a dov-unicode-10 branch from the dov-unicode63 branch. This is basically a rename.
   - [X] Merge from unicode-10 ino dov-unicode-10.
   - Examine errors:
   | Line | Reason |
   |------+--------|
   |   60 |        |
   ...
   |  279 |        |
   |  281 |        |
   - Start with 101 which is only 4 chars long.
#+BEGIN_EXAMPLE
failure on line 101
input is: 05D0 2067 202A 0041;1;1;1 1 x 4;3 1 0
base dir: RTL
expected levels: 1 1 x 4
returned levels: 1 1 1 2
...
env CFLAGS='-fPIC -g -O0' ./configure --prefix=/usr/local --with-pic
#+END_SRC   
   - Compile test-character with scons (doesn't really matter, except the bin target is different for gdb)
   - gdb on ~test-character~.
   - Run in gdb:
#+BEGIN_EXAMPLE
run --debug line101.txt
#+END_EXAMPLE      
** Comment
   - The ~bidi-type.tab.i~ wasn't rebuild by make clean, make, etc!
   - This solved all errors! Moving back to unicode-10.
* <2018-07-20 Fri>
** Bug in character
   - Got mail from Nico Rieck who spotted that I'm erronously skipping all the RTL tests. Turning these back on yields 18 errors!
   - "Last error" (line 230): ~05D0 0029 0020 0028 0661 0029;0;0;1 1 1 1 2 1;5 4 3 2 1 0~
     - No override characters!
     - Probably the algo is missing the RTL influence of the second 0029 (LPAR), which should sandwhich the 2nd and 3rd characters to level 1!
* <2018-07-22 Sun>
** Continue investigate
#+begin_src sh
gdb BidiCharacterTest
run line230.txt
#+end_src
  - It seems this fails because 05D0 is not recognized as strong! Why?
  - Ok, the problems were the bracket property was not removed after N0. Resolved. :-)
  - Down to 16 errors.
  - Now have a compact block from lines 65-80
